...
arguments(Input)
    ...
    opts.Tolerance (1,1) {mustBeNumericOrLogical, mustBeNonnegative, mustBeFinite} = false;
end
...
if(opts.Tolerance)
    adata1a = (abs(I1-m) > opts.Tolerance);
else
    adata1a = (I1~=m);
end
...
if(isempty(I2))
    ...
    if(opts.Tolerance)
    adata0 = (abs(I1-I2) > opts.Tolerance);
    adata2a = (abs(I2-m) > opts.Tolerance);
    else
        adata0 = (I1~=I2);
        adata2a = (I2~=m);
    end
