                ...
                if app.Analyzer.APQueue2.QueueLength % || app.Analyzer.HCQueue.QueueLength %% TODO
                    ...
                    try
                        ...
                        while ql && ~d.CancelRequested
                            ...
                            if d.CancelRequested || (tel > minutes(1)) % TODO: Confirm dialog if still Processing
                                break;
                ...
                if (app.Analyzer.FinishedQueue.QueueLength>0) && any(strcmp("running", {app.Analyzer.AnalysisFutures.State})) % || app.Analyzer.HCQueue.QueueLength %% TODO
                    ...
                    try
                        while (ql>0) && ~d.CancelRequested && any(strcmp("running", {app.Analyzer.AnalysisFutures.State}))
                            % TODO: Also check for changing of AnalysisFutures / check if nonempty & status...
                            pause(0.1);
                            if d.CancelRequested || (datetime('now')-t0 > seconds(20)) % TODO: Confirm dialog if still Processing
                                break;
...
%             end
            ...
            try
                updateDatastores(app, app.AnalysisParams.dpIdx0+1);
            catch ME0
    ...
    methods(Access=public)
        function cleanDataTables(app)
            try
                ...
                fprintf('[stopRecording] Error "%s" encountered while trying to sort the index-based data table: %s\n', ...
                    ME.identifier, getReport(ME));
                sorted1 = false;
            end
            ...
            try
                ...
                fprintf('[stopRecording] Error "%s" encountered while trying to sort the time-based data table: %s\n', ...
                    ME.identifier, getReport(ME));
                sorted2 = false;
            end
            ...
            try
                if ~isempty(app.DataTable{1}) && (sorted1 || sorted2)
                    if sorted1
                        if sorted1 && sorted2 && (size(app.DataTable{1},1) > size(app.DataTable{2},1))
                            ...
                            fprintf('[stopRecording] Discard mask: %s\n', strip(formattedDisplayText(msk')));
                            app.DataTable{1}(msk, :) = [];
                        end
                    ...
                    if ~isempty(app.DataTable{1}.Properties.UserData)
                        fprintf('[stopRecording] Checking for erroneous TimeZero...\n');
                        ...
                        if ~(isempty(app.DataTable{1}.Properties.UserData{1}) || ismissing(app.DataTable{1}.Properties.UserData{1}))...
                            ...
                            fprintf('[stopRecording] Corrected erroneous TimeZero.\n');
                        else
                            fprintf('[stopRecording] TimeZero is not erroneous; no corrections needed.\n');
                        end
                    end
                ...
                fprintf('[stopRecording] Error "%s" while cleaning up datatables: %s', ME.identifier, getReport(ME));
            end
            ...
            try
                    ...
                    if  ~isempty(app.DataTable{3})
                        for relTime=app.DataTable{3}.RelTime
                            if ismember(relTime, app.DataTable{2}.RelTime)
                                ...
                                fprintf('[stopRecording] WARNING: Could no longer find relTime "%s" in the datatable. Removing corresponding row from discontinuity table.', ...
                ...
                fprintf('[stopRecording] Error "%s" while updating discontinuity table: %s', ME.identifier, getReport(ME));
            end
            ...
            try
                ...
                fprintf('[stopRecording] Error "%s" encountered while trying to update paging: %s\n', ...
                    ME.identifier, getReport(ME));
            end
            ...
            try
                ...
                fprintf('[stopRecording] Error "%s" encountered while trying to set selected index: %s\n', ...
                    ME.identifier, getReport(ME));
            end
        end
    end
