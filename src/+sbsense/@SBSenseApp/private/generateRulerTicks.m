function [minTicks,majTicks,majLabels] = generateRulerTicks(timeZero, axisModeIndex, ~, rulerLims, minUnit, majUnitInfo)
    ...
    if numericMode
        minTicks = sbsense.utils.colonspace1([0.01, 0.5], rulerLims(1),uint64(minUnit),rulerLims(2));
        if isempty(majUnitInfo)
            return;
        end
        majTicks = sbsense.utils.colonspace1([0.1 0.5], rulerLims(1),uint64(majUnitInfo{1}),rulerLims(2));
    ...
    elseif iscell(majUnitInfo) && ~isnan(majUnitInfo{2}) && startsWith(majUnitInfo{4},'%')
        minTicks = timesbsense.utils.colonspace1([0.01, 0.5], rulerLims(1),seconds(minUnit),rulerLims(2));
        if isempty(majUnitInfo)
            return;
        end
...
%             soh = seconds(0);
%         end
        majTicks = timesbsense.utils.colonspace1([0.1 0.5], rulerLims(1),seconds(majUnitInfo{1}),rulerLims(2));
        if length(majTicks) > 10
            ivl = ceil(10\length(majTicks));
            idxs = sbsense.utils.colonspace(1,ivl,length(majTicks));
            majTicks = majTicks(idxs);
        ...
        if bitand(axisModeIndex,1) % Relative time
            minTicks = sbsense.utils.colonspace1([0.01, 0.5], rulerLims(1),minUnit,rulerLims(2));
            if isempty(majUnitInfo)
                return;
            end
            majTicks = sbsense.utils.colonspace1([0.1 0.5], rulerLims(1),majUnit,rulerLims(2));
        else % Absolute time
            minTicks = timesbsense.utils.colonspace1([0.01, 0.5], rulerLims(1),minUnit,rulerLims(2));
            if isempty(majUnitInfo)
                return;
            end
            majTicks = timesbsense.utils.colonspace1([0.1 0.5], rulerLims(1),majUnit,rulerLims(2));
        end
        if length(majTicks) > 10
            ivl = ceil(10\length(majTicks));
            idxs = sbsense.utils.colonspace1([0.08 0.5], 1,ivl,length(majTicks));
            majTicks = majTicks(idxs);
        end
